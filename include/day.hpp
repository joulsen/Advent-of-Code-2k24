...
public:
    ...
    void register_day(unsigned int day_number, Creator creator)
    {
        creators[day_number] = creator;
    }
    ...
    std::map<unsigned int, Creator> get_all_days() const
    {
        return creators;
    }
...
public:
    DayRegistrar()
    {
        DayFactory::instance().register_day(DayNumber, []()
                                            { return std::make_unique<T>(); });
    }
};

#define REGISTER_DAY(ClassName, DayNumber) \
    static DayRegistrar<ClassName, DayNumber> ClassName##_registrar;
